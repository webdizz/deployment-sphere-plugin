		...
		for (var index = 0; envList.length > index ; index++){
			...
			collapsable = (envList.length > 1) ? true : false;
			
			var headerDiv = createEnvHeaderDiv(envList[index], collapsable);
			envDiv.appendChild(headerDiv);
			

			if (envList[index].deployments.length>0){
				
				var buildTableDiv = createDeploymentTableDiv(envList[index].deployments, collapsable);
				envDiv.appendChild(buildTableDiv);
